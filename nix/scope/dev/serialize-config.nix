...
let
    ...
    buildRustPackageIncrementally rec {
      ...
      rootCrate = crateUtils.mkCrate {
        ...
        nix.local.dependencies = [
          globalCrates.icecap-config-cli-core
  ...
  serializers = {
    ...
    host-vmm = mk {
      ...
      crate = globalCrates.icecap-host-vmm-config;
    };
    realm-vmm = mk {
      name = "realm-vmm";
      type = "icecap_realm_vmm_config::Config";
      crate = globalCrates.icecap-realm-vmm-config;
    };
    resource-server = mk {
      name = "resource-server";
      type = "icecap_resource_server_config::Config";
      crate = globalCrates.icecap-resource-server-config;
    };
    event-server = mk {
      name = "event-server";
      type = "icecap_event_server_config::Config";
      crate = globalCrates.icecap-event-server-config;
    };
    benchmark-server = mk {
      name = "benchmark-server";
      type = "icecap_benchmark_server_config::Config";
      crate = globalCrates.icecap-benchmark-server-config;
    };
  };
