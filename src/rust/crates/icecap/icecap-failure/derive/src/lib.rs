...
fn fail_derive_impl(s: synstructure::Structure) -> Result<TokenStream, Error> {
    ...
    let cause_body = s.each_variant(|v| {
        if let Some(cause) = v.bindings().iter().find(is_cause) {
            quote!(return Some(::icecap_failure::AsFail::as_fail(#cause)))
        } else {
            quote!(return None)
        }
    });
    ...
    let fail = s.unbound_impl(
        ...
        quote! {
            ...
            fn cause(&self) -> ::core::option::Option<#make_dyn(::icecap_failure::Fail)> {
                match *self { #cause_body }
                None
            }

            #[allow(unreachable_code)]
            fn backtrace(&self) -> ::core::option::Option<&::icecap_failure::Backtrace> {
                match *self { #bt_body }
                None
            }
        },
