    ...
    pub fn retype(
        ...
        Error::wrap(unsafe {
            sys::seL4_Untyped_Retype(
                self.raw(),
                blueprint.ty() as Word,
                blueprint.raw_size_bits().unwrap_or(0) as Word,
                dst.root.raw(),
                dst.path.cptr.raw(),
                dst.path.depth as Word,
                dst_offset,
                num_objects,
            )
        })
...
impl TCB {
    pub fn read_registers(&self, suspend: bool, count: Word) -> Result<UserContext> {
        ...
        let err = unsafe {
            sys::seL4_TCB_ReadRegisters(
                self.raw(),
                suspend as sys::seL4_Bool,
                0,
                count,
                regs.raw_mut(),
            )
    ...
    pub fn write_registers(&self, resume: bool, count: Word, regs: &mut UserContext) -> Result<()> {
        Error::wrap(unsafe {
            sys::seL4_TCB_WriteRegisters(
                self.raw(),
                resume as sys::seL4_Bool,
                0,
                count,
                regs.raw_mut(),
            )
    ...
    pub fn resume(&self) -> Result<()> {
        Error::wrap(unsafe { sys::seL4_TCB_Resume(self.raw()) })
    }

    pub fn suspend(&self) -> Result<()> {
        Error::wrap(unsafe { sys::seL4_TCB_Suspend(self.raw()) })
    }
    ...
    pub fn configure(
        ...
        Error::wrap(unsafe {
            sys::seL4_TCB_Configure(
                ...
                fault_ep.raw(),
                cspace_root.raw(),
                cspace_root_data.raw(),
                vspace_root.raw(),
                0, /* HACK */
                ipc_buffer,
                ipc_buffer_frame.raw(),
            )
        })
    }
    ...
    pub fn set_affinity(&self, affinity: Word) -> Result<()> {
        Error::wrap(unsafe { sys::seL4_TCB_SetAffinity(self.raw(), affinity) })
    }

    pub fn set_tls_base(&self, tls_base: Word) -> Result<()> {
        Error::wrap(unsafe { sys::seL4_TCB_SetTLSBase(self.raw(), tls_base) })
    }

    pub fn bind_notification(&self, notification: Notification) -> Result<()> {
        Error::wrap(unsafe { sys::seL4_TCB_BindNotification(self.raw(), notification.raw()) })
    }
}

impl VCPU {
    pub fn set_tcb(&self, tcb: TCB) -> Result<()> {
        Error::wrap(unsafe { sys::seL4_ARM_VCPU_SetTCB(self.raw(), tcb.raw()) })
    }

    pub fn read_regs(&self, field: VCPUReg) -> Result<Word> {
        let res = unsafe { sys::seL4_ARM_VCPU_ReadRegs(self.raw(), field as Word) };
        Error::or(res.value, res.error as u32)
    }

    pub fn write_regs(&self, field: VCPUReg, value: Word) -> Result<()> {
        Error::wrap(unsafe { sys::seL4_ARM_VCPU_WriteRegs(self.raw(), field as Word, value) })
    }

    pub fn ack_vppi(&self, irq: Word) -> Result<()> {
        Error::wrap(unsafe { sys::seL4_ARM_VCPU_AckVPPI(self.raw(), irq) })
    }
...
impl VSpaceBranch for PUD {
    fn map(&self, vspace: PGD, vaddr: usize, attrs: VMAttributes) -> Result<()> {
        Error::wrap(unsafe {
            sys::seL4_ARM_PageUpperDirectory_Map(
                self.raw(),
                vspace.raw(),
                vaddr as u64,
                attrs.raw(),
...
impl IRQHandler {
    pub fn ack(&self) -> Result<()> {
        Error::wrap(unsafe { sys::seL4_IRQHandler_Ack(self.raw()) })
    }

    pub fn set_notification(&self, notification: Notification) -> Result<()> {
        Error::wrap(unsafe { sys::seL4_IRQHandler_SetNotification(self.raw(), notification.raw()) })
    }

    pub fn clear(&self) -> Result<()> {
        Error::wrap(unsafe { sys::seL4_IRQHandler_Clear(self.raw()) })
    }
}

impl ASIDPool {
    pub fn assign(&self, pd: PGD) -> Result<()> {
        Error::wrap(unsafe { sys::seL4_ARM_ASIDPool_Assign(self.raw(), pd.raw()) })
    }
}
...
impl RelativeCPtr {
    pub fn revoke(&self) -> Result<()> {
        Error::wrap(unsafe {
            sys::seL4_CNode_Revoke(self.root.raw(), self.path.cptr.raw(), self.path.depth as u8)
        })
    }

    pub fn delete(&self) -> Result<()> {
        Error::wrap(unsafe {
            sys::seL4_CNode_Delete(self.root.raw(), self.path.cptr.raw(), self.path.depth as u8)
        })
    }
