...
impl ClientNode {
    pub(crate) fn new(config: &ClientNodeConfig) -> Self {
        Self {
            nfn: config.nfn.clone(),
            bitfield: unsafe { Bitfield::new(config.bitfield) },
        }
    }
}
...
impl EventServerConfig {
    pub fn realize(&self) -> EventServer {
            ...
            in_spaces: (0..NUM_NODES)
                .map(|node_index| {
                    Arc::new(RefCell::new(InSpace {
                        ...
                        notification: ClientNode::new(&self.host_notifications[node_index]),
        ...
        let mut inactive_realms: BTreeMap<usize, InactiveRealm> = (0..NUM_REALMS)
            .map(|realm_id| {
                (
                    realm_id,
                    InactiveRealm {
                        out_space: mk_out_space(events::RealmOut::CARDINALITY),
                        in_notifications: self.realm_notifications[realm_id]
                            .iter()
                            .map(|config| ClientNode::new(config))
                            .collect(),
        ...
        for (out_index, event) in resource_server.out_space.iter().enumerate() {
            match events::ResourceServerOut::from_nat(out_index) {
                events::ResourceServerOut::HostRingBuffer => Event::connect(
                    event,
        ...
        for (out_index, event) in serial_server.out_space.iter().enumerate() {
            match events::SerialServerOut::from_nat(out_index) {
                events::SerialServerOut::RingBuffer(ring_buffer) => match ring_buffer {
                    events::SerialServerRingBuffer::Host => Event::connect(
                        event,
                        &host.in_spaces[PRIMARY_HOST_NODE],
                        events::HostIn::RingBuffer(events::HostRingBufferIn::SerialServer).to_nat(),
                    ),
                    events::SerialServerRingBuffer::Realm(realm_id) => {
                        inactive_realms.get_mut(&realm_id.0).unwrap().in_entries
                            [events::RealmIn::RingBuffer(
                                events::RealmRingBufferIn::SerialServer,
                            )
                            .to_nat()] = Some(event.clone());
                    }
                },
            }
        }
            ...
            match events::HostOut::from_nat(out_index) {
                events::HostOut::RingBuffer(ring_buffer) => match ring_buffer {
                    events::HostRingBufferOut::Realm(realm_id, realm_ring_buffer_id) => {
                        inactive_realms.get_mut(&realm_id.0).unwrap().in_entries
                            [events::RealmIn::RingBuffer(events::RealmRingBufferIn::Host(
                                realm_ring_buffer_id,
                            ))
                            .to_nat()] = Some(event.clone());
                    }
                },
            ...
            for (out_index, event) in inactive_realm.out_space.iter().enumerate() {
                match events::RealmOut::from_nat(out_index) {
                    events::RealmOut::RingBuffer(ring_buffer) => match ring_buffer {
                        events::RealmRingBufferOut::Host(realm_ring_buffer_id) => Event::connect(
                            event,
                            &host.in_spaces[PRIMARY_HOST_NODE],
                            events::HostIn::RingBuffer(events::HostRingBufferIn::Realm(
                                events::RealmId::from_nat(*realm_id),
                                realm_ring_buffer_id,
                            ))
                            .to_nat(),
                        ),
        ...
        let resource_server_subscriptions = self
            .resource_server_subscriptions
            .iter()
            .map(|notification| SubscriptionEntry {
        ...
        let host_subscriptions = (0..NUM_NODES)
            .map(|node_index| {
                ...
                Event::connect(
                    &event,
                    &host.in_spaces[node_index],
                    events::HostIn::RealmEvent.to_nat(),
                );
                SubscriptionEntry {
